...
abstract class Storage {
    ...
    void addToRS(int before, int after) {
        // check if a file exists
        File checkSave = new File("hydro.sav");
    ...
    void addToBLS(int before, int after) {
        // check if a file exists
        File checkSave = new File("hydro.sav");
        boolean exists = checkSave.exists();
        if(exists==false) saveStats(RS, BLS, shipments);
    ...
    void addToShipments(int before, int after) {
        // check if a file exists
        File checkSave = new File("hydro.sav");
        boolean exists = checkSave.exists();
        if(exists==false) saveStats(RS, BLS, shipments);
        openStats();
        // then create temp array to hold new values
        int[][] temp = new int[shipments[0].length + 1][BLS.length + 1];
    ...
    void saveStats(int[][] rs, int[][] bls, int[][] ship) {
        try {
            FileOutputStream saveFile = new FileOutputStream("hydro.sav"); // creates save file
    ...
    void openStats() {      
        try {
            FileInputStream saveFile = new FileInputStream("hydro.sav"); // open save file
